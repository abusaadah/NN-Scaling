...
with graph.as_default():
    profile_2 = pf.profile_coefficient(0.4, 100)
    X = tf.placeholder(tf.float32, [None, n_input])
    Y = tf.placeholder(tf.float32, [None, n_classes])
    parameters = MLP1.initialize_param(784, 100, 100, 10)
    logist_2 = MLP1.mlp(X, profile_2, parameters)
    loss_op_2, optimizer_2 = MLP1.optimize_param(logist_2, Y, learning_rate)
...
with tf.Session(graph = graph) as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            c_2,_ = sess.run([loss_op_2, optimizer_2], feed_dict = {X: batch_x, Y: batch_y})
                    
            # Compute average losses
            avg_cost2 += c_2 / total_batch
            cost_2.append(avg_cost2)
            if i % 5000 == 0:
                    pred = tf.nn.softmax(logist_2)  # Apply softmax to logits
        ...
        if epoch % display_step == 0:
            print("Epoch_2:", '%03d' % (epoch + 1), "cost = {:.9f}".format(avg_cost2))
        
    new_saver.save(sess, "my_model2")
    new_saver.export_meta_graph("my_model2.meta")
    ...
    def test_accuracy(test_accuracy_infr):
        for l in percentage_channel:
            if l % display_step == 0: 
                ...
                return print("Percentage_Profile2 Test Accuracy = " +Test_Accuracy)
    test_accuracy(test_accuracy_infr)

sess.close()

print("Second Profile Training with Fisrt Profile fixed finished")
